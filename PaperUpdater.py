...
### Paper Updating to MongoDB with Some Additional Parameters ###


paperAlreadyExist = []
for obj in collection.find({"type": "paper"}):
    paperAlreadyExist.append(obj['link'])

newPapersUploaded = 0

for i in data:
    linkAlreadyDB = i['link']

    if linkAlreadyDB not in paperAlreadyExist:
        i['totalComments'] = 0
        i['totalViews'] = 0
        i['totalLikes'] = 0
        i['type'] = 'paper'
        ...
        try:
            ...
            print(i['pdf_url'], "Already Exist!")
            continue

with open(logFilename, 'a') as f:
    print("\n\nTotal Number of New Papers: ", newPapersUploaded, file=f)
